...
void mo_with_updates(int n, const vector<Query> &queries, Add add, Rem rem, Update update, Get get) {
  ...
  for (const auto &query : gets) {
    ...
    while (cur_update + 1 < (int)updates.size() && updates[cur_update + 1].id < query.id) {
      ++cur_update;
