// Requires rollback DSU: same_component, join and rollback.
// Index from 0
//
...
void dfs(int i, int l, int r, int current_time, int n_connected_components, DSU &dsu, vector<int> &res) {
  for (auto [u, v] : edges[i]) {
    if (!dsu.same_component(u, v)) { --n_connected_components; }
...
vector<int> solve(int n, const vector<Query> &_queries) {
  ...
  for (const auto &[e, startTime] : addTimes) { update(1, 0, q - 1, startTime, q - 1, e); }
