...
// 3. Queries
//
// Requires CompressorBuilder with build() returning size() and must_geq().
...
template <typename T> struct Fenwick2D {
  vector<T> solve(vector<Query<T>> queries) {
    ...
    f.resize(sx + 1);
    for (const auto &query : queries) {
      if (query.typ == Query<T>::ADD) { fakeUpdate(query.x, query.y); }
    }
    initNodes();
    // answer queries
    vector<T> res;
    for (const auto &query : queries) {
      if (query.typ == Query<T>::ADD) {
        ...
        res.push_back(+get(query.x2 - 1, query.y2 - 1) - get(query.x2 - 1, query.y - 1) -
                      get(query.x - 1, query.y2 - 1) + get(query.x - 1, query.y - 1));
      }
    }
    return res;
  }
  ...
  void fakeUpdate(int x, int y) {
    for (; x <= sx; x += x & -x) nodes[x].push_back(y);
  }
  // point (u, v) += val
  void update(int u, int v, int val) {
    for (int x = u; x <= sx; x += x & -x)
      for (int y = lower_bound(nodes[x].begin(), nodes[x].end(), v) - nodes[x].begin() + 1; y <= (int)nodes[x].size();
           y += y & -y)
        f[x][y] += val;
  }
