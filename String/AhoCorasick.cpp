...
private:
  struct TrieNode {
    ...
    int suffixLink;
    int cnt;
    TrieNode() : exitLink(-1), suffixLink(-1), cnt(0) { fill(nxt, nxt + 26, -1); }
  };
  vector<TrieNode> trie;
  Tree &tree; // This is the exit tree for counting occurrences
public:
  AhoCorasick(Tree &_tree) : tree(_tree) { trie.push_back(TrieNode()); }
  ...
  int getApp(int node) { return tree.getSubTree(node); }
} Aho(globalTree);
