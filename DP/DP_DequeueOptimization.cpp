...
ll cost(int l, int r) { // simulate a cost function
  ++l;
  int mid = (l + r) >> 1;
  return 1LL * a[mid] * (mid - l + 1) - (pre[mid] - pre[l - 1]) + pre[r] - pre[mid - 1] - 1LL * a[mid] * (r - mid + 1);
}
struct Item {
  int l, r, opt; // Range [l,r] where transition 'opt' is optimal
  Item(int _l = 0, int _r = 0, int _opt = 0) { l = _l, r = _r, opt = _opt; }
};
...
void solve() {
  ...
  REP(i, 1, n) {
    ...
    while (!dq.empty()) {                            // Remove transitions that are now suboptimal
      ...
      if (dp[i] + cost(i, l) < dp[opt] + cost(opt, l)) dq.pop_back(); // Transition i is better than opt at position l
      else break;
    }
    if (dq.empty()) { // New transition i dominates everything
      ...
      while (lo <= hi) { // Binary search for crossover point
        int mid = (lo + hi) >> 1;
        if (dp[i] + cost(i, mid) < dp[opt] + cost(opt, mid)) p = mid, hi = mid - 1;
        else lo = mid + 1;
      }
