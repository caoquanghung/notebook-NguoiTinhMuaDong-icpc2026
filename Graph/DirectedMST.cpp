...
// Index from 0
//
// Requires rollback DSU: getRoot, join and rollback.
using ll = long long;
struct Edge {
...
pair<ll, vector<int>> directed_mst(int n, int root, vector<Edge> &edges) {
  ...
  for (const Edge &e : edges) { heap[e.v] = merge(heap[e.v], new HeapNode{e}); }
  ll res = 0;
  ...
  for (int s = 0; s < n; ++s) {
    ...
    while (seen[u] < 0) {
      ...
      if (seen[u] == s) {
        ...
        int end = qi;
        int time = dsu_time;
        do { cyc = merge(cyc, heap[w = path[--qi]]); } while (dsu.join(u, w, ++dsu_time));
        u = dsu.getRoot(u);
        heap[u] = cyc;
        seen[u] = -1;
