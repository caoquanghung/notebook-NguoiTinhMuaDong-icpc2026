...
struct UndirectedDfs {
  ...
  UndirectedDfs(const vector<vector<int>> &_g)
      : g(_g), n(g.size()), low(n, 0), num(n, -1), parent(n, 0), is_articulation(n, false), counter(0), children(0) {
    for (int u = 0; u < n; u++) {
      for (int v : g[u]) { cnt_edges[{u, v}] += 1; }
    }
    for (int i = 0; i < n; ++i)
      if (num[i] == -1) {
        root = i;
  ...
  void dfs(int u) {
    ...
    for (int v : g[u]) {
      if (num[v] == -1) {
        ...
        if (low[v] > num[u]) {
          if (cnt_edges[{u, v}] == 1) { bridges.push_back(make_pair(u, v)); }
        }
