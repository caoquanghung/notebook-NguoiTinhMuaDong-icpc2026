...
struct Point {
  ...
  Point operator%(const Point &op) const {
    return Point(det(y, z, op.y, op.z), -det(x, z, op.x, op.z), det(x, y, op.x, op.y));
  }
};
struct Space {
  double a, b, c, d;
...
double volume(Point x, Point y, Point z) { // Thể tích của 3 vector
  Point base = Point(y.y * z.z - y.z * z.y, y.z * z.x - y.x * z.z, y.x * z.y - y.y * z.x);
...
inline bool isUsed(int i, int j, int k) { return used.find(getSideKey(i, j, k)) != used.end(); }
inline double getAngle(const Point &n1, const Point &n2) { return atan2((n1 % n2).length(), n1 * n2); }
...
void dfs(int i, int j, int k, vector<Point> &p, vector<Side> &sides) {
  ...
  used.insert(key);
  int n = p.size();
  if (!isUsed(j, k, i)) dfs(j, k, i, p, sides);
  if (!isUsed(k, i, j)) dfs(k, i, j, p, sides);
  int bestT = -1;
  ...
  if (!isUsed(i, bestT, j)) { dfs(i, bestT, j, p, sides); }
}
...
inline bool isEmpty(Point x, Point y, Point z) {
  return abs(x * Point(y.y * z.z - y.z * z.y, y.z * z.x - y.x * z.z, y.x * z.y - y.y * z.x)) <= EPS;
}
inline bool conflict(Side a, Side b) {
  Point x = arr[a[0]], y = arr[a[1]], z = arr[a[2]];
  REP(i, 3) {
    Point t = arr[b[i]];
    if (!isEmpty(x - t, y - t, z - t)) return false;
  }
  return true;
}
vector<Side> eliminate(vector<Side> p) {
  ...
  REP(i, p.size()) {
    ...
    if (fre[j]) {
      if (conflict(p[i], p[j])) {
        fre[j] = false;
        res.back().insert(res.back().end(), p[j].begin(), p[j].end());
      }
    }
  }
  REP(i, res.size()) {
